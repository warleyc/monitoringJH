...
/**
 * Performance test for the ProjectConfiguration entity.
 */
class ProjectConfigurationGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the ProjectConfiguration entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all projectConfigurations")
            .get("/api/projectConfigurations")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new projectConfiguration")
            .post("/api/projectConfigurations")
            .headers(headers_http_authenticated)
            ...
            .pause(10)
            .repeat(5) {
                exec(http("Get created projectConfiguration")
                .get("${new_projectConfiguration_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created projectConfiguration")
            .delete("${new_projectConfiguration_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
